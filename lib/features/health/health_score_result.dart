...
class HealthScoreResult {
  ...
  const HealthScoreResult({required this.gesamtScore, required this.faktoren});
...
String scoreLabel(int score, [AppLocalizations? l10n]) {
  ...
  if (score >= 40) {
    return l10n?.healthScoreLabelVerbesserung ?? 'Verbesserungsbedarf';
  }
...
String motivationstext(int score, [AppLocalizations? l10n]) {
  if (score >= 85) {
    return l10n?.motivationExcellent ??
        'Ausgezeichnet! Dein Körper ist in Topform.';
  }
  if (score >= 70) {
    return l10n?.motivationGood ??
        'Gut unterwegs! Kleine Optimierungen bringen dich weiter.';
  }
  if (score >= 55) {
    return l10n?.motivationSolid ??
        'Solide Basis. Fokussiere dich auf deine schwächsten Bereiche.';
  }
  if (score >= 40) {
    return l10n?.motivationImprove ??
        'Es gibt Verbesserungspotenzial. Schau dir die Empfehlungen an.';
  }
  return l10n?.motivationAttention ??
      'Dein Körper braucht Aufmerksamkeit. Starte mit kleinen Schritten.';
}
...
String faktorHinweis(String name, [AppLocalizations? l10n]) {
  switch (name) {
    case 'Training':
      return l10n?.hintTraining ??
          'Plane dein nächstes Workout und bleib aktiv.';
    case 'Ernährung':
      return l10n?.hintNutrition ??
          'Tracke deine Mahlzeiten und triff dein Kalorienziel.';
    case 'Regeneration':
      return l10n?.hintRegeneration ??
          'Achte auf 7–9 Stunden Schlaf pro Nacht.';
    case 'Supplemente':
      return l10n?.hintSupplements ?? 'Nimm deine heutigen Supplemente ein.';
    case 'Medikamente':
      return l10n?.hintMedication ??
          'Vergiss nicht, deine Medikamente zu nehmen.';
    case 'Stress & Mental':
      return l10n?.hintMentalStress ??
          'Notiere deine heutige Stimmung und mach eine Pause.';
    default:
      return l10n?.hintDefault ?? 'Schau dir die Details an.';
  }
}
