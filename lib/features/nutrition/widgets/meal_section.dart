...
class MealSection extends ConsumerWidget {
  ...
  double get _totalCal => entries.fold(0.0, (s, e) => s + e.calories);
  double get _totalProtein => entries.fold(0.0, (s, e) => s + e.protein);
  double get _totalCarbs => entries.fold(0.0, (s, e) => s + e.carbs);
  double get _totalFat => entries.fold(0.0, (s, e) => s + e.fat);
      ...
      child: Column(
        ...
        children: [
          ...
          if (entries.isEmpty)
            ...
            const Divider(color: TraumColors.surfaceVariant, height: 16),
            Text(
  ...
  void _openSheet(BuildContext ctx) {
    showModalBottomSheet(
      ...
      builder: (_) => MealTemplateSheet(mealType: mealType, date: date),
    );
  }

  Future<void> _openScanner(BuildContext ctx, WidgetRef ref) async {
    ...
    if (product != null && ctx.mounted) {
      showModalBottomSheet(
        ...
        backgroundColor: Colors.transparent,
        builder: (_) =>
            AmountEntrySheet(product: product, mealType: mealType, date: date),
      );
    }
  }
}
  ...
  Widget build(BuildContext context) {
    return Dismissible(
      ...
      background: Container(
        ...
        child: const Icon(Icons.delete_outline, color: TraumColors.roseRed),
      ),
      onDismissed: (_) async {
        await ref.read(mealEntriesDaoProvider).deleteEntry(entry.id);
