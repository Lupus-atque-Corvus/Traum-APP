...
export '../../core/providers/preferences_provider.dart'
    show activeDiaryProvider;
...
final diaryEntryCountProvider = FutureProvider.autoDispose.family<int, int>(
  (ref, diaryId) => ref.watch(diaryRepositoryProvider).diaryEntryCount(diaryId),
);
...
final diaryGhostImageProvider = FutureProvider.autoDispose<String?>((
  ref,
) async {
...
final todaysDiaryEntryProvider = FutureProvider.autoDispose<DiaryEntry?>((ref) {
  final diaryId = ref.watch(activeDiaryProvider);
...
final datesWithDiaryEntriesProvider = FutureProvider.autoDispose<Set<String>>((
  ref,
) async {
  final diaryId = ref.watch(activeDiaryProvider);
  final dates = await ref
      .watch(diaryRepositoryProvider)
      .getDatesWithEntries(diaryId);
  return dates.toSet();
});
