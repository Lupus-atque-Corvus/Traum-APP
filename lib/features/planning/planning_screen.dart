...
class _PlanningScreenState extends ConsumerState<PlanningScreen>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: const [_CalendarTab(), _TodosTab()],
      ),
    );
  }
}
...
class _CalendarTabState extends ConsumerState<_CalendarTab> {
  ...
  void initState() {
    super.initState();
    _calendarFormat =
        ref.read(preferencesRepositoryProvider).planningCalendarFormat == 'week'
        ? CalendarFormat.week
            ...
            data: (appts) {
              ...
              return TableCalendar<Appointment>(
                ...
                availableCalendarFormats: {
                  CalendarFormat.month: AppLocalizations.of(
                    context,
                  )!.periodMonth,
            ...
            child: dayApptAsync.when(
              data: (appts) {
                if (appts.isEmpty) {
                  return Center(
                    child: Text(
                      AppLocalizations.of(context)!.noAppointmentsOnDate(
                    ...
                    return Dismissible(
                      ...
                      confirmDismiss: (_) => confirmDeleteDialog(
                        ...
                        title: AppLocalizations.of(
                          context,
                        )!.deleteAppointmentConfirmTitle,
                        content: AppLocalizations.of(
                          context,
                        )!.deleteAppointmentConfirmContent(a.title),
                      ),
                      onDismissed: (_) => ref
                          .read(calendarSyncServiceProvider)
                          .deleteAppointmentWithSync(a.id),
...
class _AddAppointmentSheetState extends State<_AddAppointmentSheet> {
  ...
  void initState() {
    ...
    if (existing != null) {
      ...
      _endTime =
          existing.endTime ?? existing.startTime.add(const Duration(hours: 1));
    } else {
      _startTime = DateTime(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _buildField(
              AppLocalizations.of(context)!.titleRequiredField,
              _titleCtrl,
            ),
              ...
              onPressed: () async {
                ...
                if (_endTime.isBefore(_startTime)) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        AppLocalizations.of(context)!.endBeforeStartError,
                      ),
                    ),
                  );
                  return;
  ...
  Widget build(BuildContext context) {
    ...
    return Dismissible(
      ...
      confirmDismiss: (_) => confirmDeleteDialog(
        ...
        content: AppLocalizations.of(
          context,
        )!.deleteTodoConfirmContent(todo.title),
      ),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text(
            AppLocalizations.of(context)!.fieldPriority,
            style: const TextStyle(
              color: TraumColors.onBackgroundMuted,
              fontFamily: 'DMSans',
              fontSize: 13,
          ...
          GradientButton(
            ...
            onPressed: () async {
              if (_titleCtrl.text.trim().isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(AppLocalizations.of(context)!.titleRequired),
                  ),
                );
                return;
              }
