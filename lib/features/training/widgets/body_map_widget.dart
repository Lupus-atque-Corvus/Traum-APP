...
class BodyMapWidget extends StatelessWidget {
  ...
  /// 0..1 per body-map muscle — overrides primary/secondary/heatMap colors
  /// when set (ADDITIVE mode, task 8.2). When null, behavior is unchanged.
  ...
  /// 0..1 → hex color between "not trained" and "heavily trained"
  /// (task 8.2 continuous-intensity mode).
  static String _intensityHex(double t) {
    final c = Color.lerp(
  ...
  String _buildSvg() {
    ...
    for (final entry in _muscleDefaultColors.entries) {
      ...
      if (intensity != null) {
        targetHex = _intensityHex(intensity![muscle] ?? 0);
