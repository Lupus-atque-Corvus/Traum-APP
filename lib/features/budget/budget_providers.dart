...
class BudgetCategoryWithSpending {
  ...
  double get ratio =>
      budgetLimit > 0 ? (spent / budgetLimit).clamp(0.0, 1.0) : 0.0;
...
final selectedTrendPeriodProvider = StateProvider<TrendPeriod>(
  (_) => TrendPeriod.month,
);
...
final categoryExpensesProvider = StreamProvider.autoDispose
    .family<List<CategoryExpense>, (int, int)>((ref, ym) {
      ...
      final cats =
          ref.watch(allBudgetCategoriesStreamProvider).value ?? const [];
    ...
    .family<List<FlSpot>, (int, int)>((ref, ym) {
      ...
      return ref.watch(budgetDaoProvider).watchAllTransactions().map((all) {
        ...
        for (final t in all.where(
          (t) => t.date.year == ym.$1 && t.date.month == ym.$2,
        )) {
...
final debtItemsStreamProvider = StreamProvider.autoDispose
    .family<List<DebtItem>, int>(
...
final trendDataProvider = StreamProvider.autoDispose
    .family<List<TrendBar>, TrendPeriod>((ref, period) {
      ...
      return dao.watchAllTransactions().map((all) {
        ...
        double income(Iterable<Transaction> txs) => txs
            .where((t) => t.type == 'income')
            .fold(0.0, (s, t) => s + t.amount);
        double expenses(Iterable<Transaction> txs) => txs
            .where((t) => t.type == 'expense')
            .fold(0.0, (s, t) => s + t.amount);
          ...
          case TrendPeriod.month:
            ...
            for (int w = 3; w >= 0; w--) {
              final weekStart = now.subtract(
                Duration(days: w * 7 + now.weekday - 1),
              );
              double inc = 0, exp = 0;
              for (int d = 0; d < 7; d++) {
              ...
              bars.add(
                TrendBar(label: 'KW${3 - w + 1}', income: inc, expenses: exp),
              );
            }
            return bars;
...
Map<int, double> deriveAccountBalances(
    ...
    switch (t.type) {
      ...
      case 'transfer':
        ...
        if (t.toAccountId != null) {
          m[t.toAccountId!] = (m[t.toAccountId!] ?? 0) + t.amount;
        }
        break;
    }
  }
...
final accountDerivedBalancesProvider =
    StreamProvider.autoDispose<Map<int, double>>((ref) {
      ...
      return ref
          ...
          .map((txs) => deriveAccountBalances(accounts, txs));
    });
...
final monthlyBalanceChangeProvider = StreamProvider.autoDispose<double?>((ref) {
  final dao = ref.watch(budgetDaoProvider);
  final now = DateTime.now();
...
final budgetCategoriesWithSpendingProvider = StreamProvider.autoDispose
    .family<List<BudgetCategoryWithSpending>, (int, int)>((ref, ym) {
      final dao = ref.watch(budgetDaoProvider);
      final cats =
          ref.watch(allBudgetCategoriesStreamProvider).value ?? const [];
      ...
      return dao.watchTransactionsForMonth(ym.$1, ym.$2).map((txs) {
        ...
        return cats
            .where(
              (c) =>
                  c.isExpense && c.monthlyLimit != null && c.monthlyLimit! > 0,
            )
            .map(
              (cat) => BudgetCategoryWithSpending(
                category: cat,
...
final recentTransactionItemsProvider = StreamProvider.autoDispose
    .family<List<RecentTransactionItem>, int>((ref, limit) {
      final dao = ref.watch(budgetDaoProvider);
      final cats =
          ref.watch(allBudgetCategoriesStreamProvider).value ?? const [];
      final catMap = {for (final c in cats) c.id: c};
