...
// ─── Supplement ───────────────────────────────────────────────────────────────
final supplementsStreamProvider = StreamProvider.autoDispose<List<Supplement>>(
  (ref) => ref.watch(supplementDaoProvider).watchAllSupplements(),
);

// ─── Abstinence ───────────────────────────────────────────────────────────────
final abstinenceTrackersStreamProvider =
    StreamProvider.autoDispose<List<AbstinenceTracker>>(
      (ref) => ref.watch(abstinenceDaoProvider).watchAllTrackers(),
    );
...
final medicationLogsForDateProvider = StreamProvider.autoDispose
    .family<List<MedicationLog>, DateTime>(
      (ref, date) => ref.watch(medicationDaoProvider).watchLogsForDate(date),
    );

// ─── Planning ─────────────────────────────────────────────────────────────────
final allAppointmentsStreamProvider =
    StreamProvider.autoDispose<List<Appointment>>(
      (ref) => ref.watch(planningDaoProvider).watchAllAppointments(),
    );

final appointmentsForDateProvider = StreamProvider.autoDispose
    .family<List<Appointment>, DateTime>(
      (ref, date) =>
          ref.watch(planningDaoProvider).watchAppointmentsForDate(date),
    );

final allTodosStreamProvider = StreamProvider.autoDispose<List<Todo>>(
  (ref) => ref.watch(planningDaoProvider).watchAllTodos(),
);

final allGoalsStreamProvider = StreamProvider.autoDispose<List<Goal>>(
  (ref) => ref.watch(planningDaoProvider).watchAllGoals(),
);

final allHabitsStreamProvider = StreamProvider.autoDispose<List<Habit>>(
  (ref) => ref.watch(planningDaoProvider).watchAllHabits(),
);

final habitLogsForDateProvider = StreamProvider.autoDispose
    .family<List<HabitLog>, DateTime>(
      (ref, date) => ref.watch(planningDaoProvider).watchHabitLogsForDate(date),
    );

final habitLogsLast7DaysProvider = FutureProvider.autoDispose
    .family<List<HabitLog>, int>(
      (ref, habitId) =>
...
final allBudgetCategoriesStreamProvider =
    StreamProvider.autoDispose<List<BudgetCategory>>(
      (ref) => ref.watch(budgetDaoProvider).watchAllCategories(),
    );

final transactionsForMonthProvider = StreamProvider.autoDispose
    .family<List<Transaction>, (int, int)>(
      (ref, ym) =>
          ref.watch(budgetDaoProvider).watchTransactionsForMonth(ym.$1, ym.$2),
    );

// ─── Nutrition ────────────────────────────────────────────────────────────────
final nutritionLogsForDateProvider = StreamProvider.autoDispose
    .family<List<NutritionLog>, DateTime>(
      (ref, date) => ref.watch(nutritionDaoProvider).watchLogsForDate(date),
    );

final waterForDateProvider = StreamProvider.autoDispose
    .family<List<WaterLog>, DateTime>(
      (ref, date) => ref.watch(nutritionDaoProvider).watchWaterForDate(date),
    );

// ─── Profile / Health ─────────────────────────────────────────────────────────
final latestWeightProvider = FutureProvider.autoDispose<WeightLog?>(
  (ref) => ref.watch(healthDaoProvider).getLatestWeight(),
);

final recentSleepLogsProvider = FutureProvider.autoDispose
    .family<List<SleepLog>, int>(
      (ref, days) => ref.watch(healthDaoProvider).getRecentSleepLogs(days),
    );

final latestMoodProvider = FutureProvider.autoDispose<MoodLog?>(
...
final trainingSessionsThisWeekProvider =
    FutureProvider.autoDispose<List<WorkoutSession>>(
      (ref) => ref.watch(trainingDaoProvider).getSessionsThisWeek(),
    );

final allWeightLogsStreamProvider = StreamProvider.autoDispose<List<WeightLog>>(
  (ref) => ref.watch(healthDaoProvider).watchAllWeightLogs(),
);

final allSleepLogsStreamProvider = StreamProvider.autoDispose<List<SleepLog>>(
  (ref) => ref.watch(healthDaoProvider).watchAllSleepLogs(),
);

final allMeasurementsStreamProvider =
    StreamProvider.autoDispose<List<BodyMeasurement>>(
      (ref) => ref.watch(healthDaoProvider).watchAllMeasurements(),
    );

final allMoodLogsStreamProvider = StreamProvider.autoDispose<List<MoodLog>>(
  (ref) => ref.watch(healthDaoProvider).watchAllMoodLogs(),
);

// ─── Training ─────────────────────────────────────────────────────────────────
final allExercisesStreamProvider = StreamProvider.autoDispose<List<Exercise>>(
  (ref) => ref.watch(trainingDaoProvider).watchAllExercises(),
);

final allWorkoutPlansStreamProvider =
    StreamProvider.autoDispose<List<WorkoutPlan>>(
      (ref) => ref.watch(trainingDaoProvider).watchAllPlans(),
    );

final workoutDaysForPlanProvider = StreamProvider.autoDispose
    .family<List<WorkoutDay>, int>(
      (ref, planId) => ref.watch(trainingDaoProvider).watchDaysForPlan(planId),
    );

final setsForSessionProvider = StreamProvider.autoDispose
    .family<List<WorkoutSet>, int>(
      (ref, sessionId) =>
...
final dayExercisesProvider = StreamProvider.autoDispose
    .family<List<WorkoutDayExercise>, int>(
      (ref, dayId) => ref.watch(trainingDaoProvider).watchDayExercises(dayId),
    );

final activePlanProvider = FutureProvider.autoDispose<WorkoutPlan?>(
  (ref) => ref.watch(trainingDaoProvider).getActivePlan(),
);

// ─── Daily Routines (morning / evening) ──────────────────────────────────────
/// Plans of a given planType ('workout' | 'morning' | 'evening').
/// Family param is a primitive String — allowed per Riverpod constraint.
final routinesByTypeProvider = StreamProvider.autoDispose
    .family<List<WorkoutPlan>, String>(
      (ref, type) => ref.watch(trainingDaoProvider).watchPlansByType(type),
    );

final morningRoutinesProvider = StreamProvider.autoDispose<List<WorkoutPlan>>(
  (ref) => ref.watch(trainingDaoProvider).watchPlansByType('morning'),
);

final eveningRoutinesProvider = StreamProvider.autoDispose<List<WorkoutPlan>>(
  (ref) => ref.watch(trainingDaoProvider).watchPlansByType('evening'),
);

/// All plans with planType != 'workout' (morning + evening combined).
final dailyRoutinesProvider = StreamProvider.autoDispose<List<WorkoutPlan>>(
  (ref) => ref.watch(trainingDaoProvider).watchDailyRoutinePlans(),
);

// ─── Nutrition extras ─────────────────────────────────────────────────────────
final allMealTemplatesStreamProvider =
    StreamProvider.autoDispose<List<MealTemplate>>(
      (ref) => ref.watch(nutritionDaoProvider).watchAllTemplates(),
    );

final allShoppingItemsStreamProvider =
    StreamProvider.autoDispose<List<ShoppingListItem>>(
      (ref) => ref.watch(nutritionDaoProvider).watchAllShoppingItems(),
    );

final shoppingTemplatesStreamProvider =
    StreamProvider.autoDispose<List<ShoppingTemplate>>(
      (ref) => ref.watch(nutritionDaoProvider).watchShoppingTemplates(),
    );

final groceryPriceEntriesProvider =
    FutureProvider.autoDispose<List<PriceEntry>>(
      (ref) => ref.watch(nutritionDaoProvider).getAllPriceEntries(),
    );

// ─── Period ───────────────────────────────────────────────────────────────────
final allPeriodEntriesStreamProvider =
    StreamProvider.autoDispose<List<PeriodEntry>>(
      (ref) => ref.watch(periodDaoProvider).watchAllPeriodEntries(),
    );

final allPeriodSymptomsStreamProvider =
    StreamProvider.autoDispose<List<PeriodSymptom>>(
      (ref) => ref.watch(periodDaoProvider).watchAllSymptoms(),
    );

final cycleProfileStreamProvider =
    StreamProvider.autoDispose<CycleProfileData?>(
      (ref) => ref.watch(periodDaoProvider).watchCycleProfile(),
    );

final allDailyLogsStreamProvider = StreamProvider.autoDispose<List<DailyLog>>(
  (ref) => ref.watch(periodDaoProvider).watchAllDailyLogs(),
);

final dailyLogForDateProvider = FutureProvider.autoDispose
    .family<DailyLog?, DateTime>(
      (ref, date) => ref.watch(periodDaoProvider).getDailyLogForDate(date),
    );
...
final allTransactionsStreamProvider =
    StreamProvider.autoDispose<List<Transaction>>(
      (ref) => ref.watch(budgetDaoProvider).watchAllTransactions(),
    );

// ─── Muscle groups for a routine plan (for body map preview) ─────────────────
final planMuscleGroupsProvider = FutureProvider.autoDispose
    .family<List<String>, int>((ref, planId) async {
...
final sessionsLast72hProvider =
    FutureProvider.autoDispose<List<WorkoutSession>>(
      (ref) => ref
          .watch(trainingDaoProvider)
          .getSessionsAfter(DateTime.now().subtract(const Duration(hours: 72))),
    );
...
final allSavingsGoalsStreamProvider =
    StreamProvider.autoDispose<List<SavingsGoal>>(
      (ref) => ref.watch(budgetDaoProvider).watchAllSavingsGoals(),
    );

final exerciseSetCountsProvider = FutureProvider.autoDispose<Map<int, int>>(
  (ref) => ref.watch(trainingDaoProvider).getExerciseSetCounts(),
);

final exerciseSessionHistoryProvider = FutureProvider.autoDispose
    .family<List<(WorkoutSession, List<WorkoutSet>)>, int>(
      (ref, exerciseId) => ref
          .watch(trainingDaoProvider)
          .getSessionsWithSetsForExercise(exerciseId),
    );

// ─── Substance ────────────────────────────────────────────────────────────────
final substanceDaoProvider = Provider<SubstanceDao>(
