...
class PeriodDao extends DatabaseAccessor<TraumDatabase> with _$PeriodDaoMixin {
  ...
  // PeriodEntries
  Stream<List<PeriodEntry>> watchAllPeriodEntries() => (select(
    periodEntries,
  )..orderBy([(t) => OrderingTerm.desc(t.startDate)])).watch();

  Future<List<PeriodEntry>> getAllPeriodEntries() => (select(
    periodEntries,
  ...
  Stream<List<PeriodSymptom>> watchSymptomsForDate(DateTime date) {
    ...
    return (select(periodSymptoms)..where(
          (t) =>
  ...
  Stream<List<PeriodSymptom>> watchAllSymptoms() => (select(
    periodSymptoms,
  ...
  Stream<List<DailyLog>> watchAllDailyLogs() => (select(
    dailyLogs,
  )..orderBy([(t) => OrderingTerm.asc(t.logDate)])).watch();
  ...
  Future<void> updateCycleProfile(CycleProfileCompanion entry) async {
    await (update(
      cycleProfile,
    )..where((t) => t.id.equals(0))).write(entry.copyWith(id: const Value(0)));
  }
}
