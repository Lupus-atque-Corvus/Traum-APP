...
SubstanceRecord _record({
  ...
  DatenStatus status = DatenStatus.vollstaendig,
}) => SubstanceRecord(
  id: 1,
  substance: name,
...
void main() {
  testWidgets('shows the category grid when the query is empty', (
    tester,
  ) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          substanceCategoriesProvider.overrideWith(
            (ref) async => ['Schmerzmittel', 'Herz-Kreislauf'],
          ),
        ],
  ...
  testWidgets('shows search results with a status badge after typing', (
    tester,
  ) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          ...
          substanceSearchProvider(
            'ibu',
          ).overrideWith((ref) async => [_record(name: 'Ibuprofen')]),
        ],
        child: const MaterialApp(
          localizationsDelegates: AppLocalizations.localizationsDelegates,
          supportedLocales: AppLocalizations.supportedLocales,
