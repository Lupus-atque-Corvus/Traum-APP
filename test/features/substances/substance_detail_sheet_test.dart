...
void main() {
  testWidgets('null fields render as "keine Angabe", not blank', (tester) async {
    await tester.pumpWidget(_wrap(Consumer(builder: (context, ref, _) {
      return ElevatedButton(
        onPressed: () => showSubstanceDetailSheet(context, ref, _record()),
  ...
  testWidgets('shows the free-text Wechselwirkungen block when present', (tester) async {
    await tester.pumpWidget(_wrap(Consumer(builder: (context, ref, _) {
      return ElevatedButton(
        onPressed: () => showSubstanceDetailSheet(
            context, ref, _record(wechselwirkungenDe: 'Erhöhtes Blutungsrisiko mit Warfarin.')),
        child: const Text('open'),
      );
    })));
    await tester.tap(find.text('open'));
    ...
    expect(find.textContaining('Erhöhtes Blutungsrisiko mit Warfarin.'), findsOneWidget);
  });

  testWidgets('shows Wikipedia attribution footer when quellenTags contains wikipedia:ww',
      (tester) async {
