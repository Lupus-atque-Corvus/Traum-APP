...
void main() {
  testWidgets('does not show an interaction banner in the widget tree', (
    tester,
  ) async {
  ...
  testWidgets(
    'long-press on a medication card opens the context menu with Löschen',
    (tester) async {
      final db = TraumDatabase.forTesting(NativeDatabase.memory());
      addTearDown(db.close);
      await db.medicationDao.insertMedication(
  ...
  testWidgets('confirming Löschen in the context menu deletes the medication', (
    tester,
  ) async {
    final db = TraumDatabase.forTesting(NativeDatabase.memory());
    addTearDown(db.close);
