...
void main() {
  testWidgets('renders SvgPicture for known muscle group', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(body: ExerciseIcon(muscleGroup: 'chest')),
      ),
    );
    expect(find.byType(SvgPicture), findsOneWidget);
  });

  testWidgets('renders SvgPicture for unknown muscle group (fallback)', (
    tester,
  ) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(body: ExerciseIcon(muscleGroup: 'unknown_group')),
      ),
    );
    expect(find.byType(SvgPicture), findsOneWidget);
  });
      ...
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: ExerciseIcon(
              muscleGroup: 'chest',
              exerciseName: 'Völlig Erfundene Übung XYZ',
            ),
          ),
        ),
      );
