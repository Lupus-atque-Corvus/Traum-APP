...
import 'package:traum/core/services/calendar_sync_service.dart' show NativeCalendar;
import 'package:traum/features/planning/calendar_picker_dialog.dart';
import 'package:traum/l10n/app_localizations.dart';

void main() {
  Widget wrap(List<NativeCalendar> calendars, List<String> initialIds,
  ...
  testWidgets('shows a hint instead of an empty list when there are no calendars',
      (tester) async {
    await tester.pumpWidget(wrap(const [], const [], (_) {}));
    await tester.tap(find.text('open'));
    await tester.pumpAndSettle();

    expect(find.text('Keine Kalender gefunden.\nBitte schließe den Planner und öffne ihn erneut.'),
        findsOneWidget);
  });
  ...
  testWidgets('Done is disabled until at least one calendar is selected',
    ...
    final doneButton =
        tester.widget<TextButton>(find.widgetWithText(TextButton, 'Fertig'));
