...
Widget _wrap(Locale locale, Widget Function(BuildContext) builder) => MaterialApp(
      locale: locale,
...
void main() {
  testWidgets('localizedFieldLabel resolves known keys per locale, unknown falls back',
      (tester) async {
    late BuildContext ctx;
    await tester.pumpWidget(_wrap(const Locale('de'), (c) {
      ctx = c;
    ...
    expect(localizedFieldLabel(ctx, 'custom_foo', 'Meine Bezeichnung'),
        'Meine Bezeichnung');

    await tester.pumpWidget(_wrap(const Locale('en'), (c) {
      ctx = c;
      return const SizedBox.shrink();
    }));
  ...
  testWidgets('localizedOptionValue resolves known raw values per locale, unknown passes through',
      (tester) async {
    late BuildContext ctx;
    await tester.pumpWidget(_wrap(const Locale('de'), (c) {
      ctx = c;
    ...
    expect(localizedOptionValue(ctx, 'Meine eigene Option'), 'Meine eigene Option');
  ...
  testWidgets(
    ...
    expect(
      localizedCollectionName(ctx, _collection(iconName: 'tower', name: 'Türme')),
      'Towers',
    );
    expect(
      localizedCollectionName(
          ctx, _collection(iconName: 'home_broken', name: 'Lost Places')),
      'Lost Places',
    );
    // Same icon as a built-in, but user-chosen name -> must NOT be overridden.
    expect(
      localizedCollectionName(
          ctx, _collection(iconName: 'tower', name: 'My Running Log')),
      'My Running Log',
    );
    // Renamed built-in collection -> respects the user's rename.
    expect(
      localizedCollectionName(ctx, _collection(iconName: 'spray', name: 'Streetart')),
      'Streetart',
    );
  });
  ...
  testWidgets('localizedTemplateDisplayName resolves all 4 fixed templates',
    ...
    expect(localizedTemplateDisplayName(ctx, MapTemplates.graffiti), 'Graffiti');
    expect(localizedTemplateDisplayName(ctx, MapTemplates.tuerme), 'Towers');
    expect(localizedTemplateDisplayName(ctx, MapTemplates.lostPlaces), 'Lost Places');
