...
void main() {
  ...
  test(
    ...
    () async {
      ...
      final stepJackPrimary = (jsonDecode(stepJack.primaryMuscles) as List)
          .cast<String>();
      expect(stepJackPrimary, contains('quadriceps'));

      // A freshly-inserted exercise decodes its muscle JSON correctly.
      final langsame = all.where((e) => e.name == 'Langsame Kniebeuge').single;
      final primary = (jsonDecode(langsame.primaryMuscles) as List)
          .cast<String>();
      expect(primary, ['quadriceps']);
      final secondary = (jsonDecode(langsame.secondaryMuscles) as List)
          .cast<String>();
