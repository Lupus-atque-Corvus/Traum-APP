...
Future<int> _insertCollection(TraumDatabase db, {String iconName = 'tower'}) {
  return db.mapCollectionsDao.insert(
...
void main() {
  ...
  group('countAll', () {
    ...
    test(
      'counts markers across all collections without loading rows',
      () async {
  ...
  group('getMostRecentByCollection', () {
    ...
    test(
      'returns the most recently created marker for that collection',
      () async {
        final id = await _insertCollection(db);
        ...
        final newestId = await db.mapMarkersDao.insert(
          MapMarkersCompanion.insert(
            collectionId: id,
  ...
  group('getByCollectionInBounds', () {
    test('only returns markers inside the given lat/lon range', () async {
      final id = await _insertCollection(db);
      Future<void> addAt(double lat, double lon) => db.mapMarkersDao.insert(
        MapMarkersCompanion.insert(
          collectionId: id,
          latitude: Value(lat),
          longitude: Value(lon),
      ...
      expect(
        result.every((m) => m.latitude! >= 51.0 && m.latitude! <= 53.0),
        isTrue,
      );
    });
  ...
  group('byHashtagSubstring', () {
    test(
      'finds markers outside any given viewport (collection-wide)',
      () async {
        final id = await _insertCollection(db);
        await db.mapMarkersDao.insert(
          MapMarkersCompanion.insert(
    ...
    test('respects the limit parameter', () async {
      ...
      final result = await db.mapMarkersDao.byHashtagSubstring(
        id,
        'art',
        limit: 4,
      );
